...
from secondguard import sg_hybrid_encrypt, sg_hybrid_encrypt_with_auditlog, sg_hybrid_decrypt, BadRequestError
...
def perform_sg_hybrid_encryption_and_decryption_with_auditlog(secret, deprecate_at=None):
    local_ciphertext, sg_recovery_instructions, sg_recovery_instructions_digest = sg_hybrid_encrypt_with_auditlog(
        to_encrypt=secret, rsa_pubkey=TESTING_RSA_PUBKEY, api_token=TESTING_API_TOKEN, deprecate_at=deprecate_at,
    )
...
def test_sg_hybrid_encryption_and_decryption():
    ...
    for deprecate_at in (None, future_expiry):
        perform_sg_hybrid_encryption_and_decryption(secret=secret, deprecate_at=deprecate_at)
        perform_sg_hybrid_encryption_and_decryption_with_auditlog(secret=secret, deprecate_at=deprecate_at)

    # Confirm that an expired key throws an error:
    try:
        perform_sg_hybrid_encryption_and_decryption(secret=secret, deprecate_at=past_expiry)
        assert False
    except BadRequestError:
        assert True

    try:
        perform_sg_hybrid_encryption_and_decryption_with_auditlog(secret=secret, deprecate_at=past_expiry)
        assert False
    except BadRequestError:
        assert True
